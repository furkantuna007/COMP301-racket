(module tests mzscheme
  ...
  (define test-list
    ...
    (let-proc "let x = 200 in let f = proc (z) -(z,x) in let x = 100 in let g = proc (z) -(z,x) in -((f 1), (g 1))" -100)
    
    (mult "let mult = proc (y) proc (x) if zero?(x) then 0 else -(((y y) -(x,1)), -10) in let multiplier = proc (x) ((mult mult) x) in (multiplier 2)" 20)

  (letrec " let z = 10 in letrec test(x) = if zero?(x) then let g = proc(x) proc (y) -(x,y) in ((g -(100,31)) 7) else (test -(x,1)) in (test 7)" 62)
