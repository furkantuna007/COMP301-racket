...
;;;;;;;;;;;;;;;; the interpreter ;;;;;;;;;;;;;;;;
      ...
      (update-vector-exp (exp1 exp2 exp3)
        ...
        (swap-vector-exp (exp1 exp2 exp3)
                         ...
                         (let ((vect (value-of exp1 env))
                           ...
                           (let ((value (deref (list-ref (expval->vec vect) index1))))

                               (begin
                               (setref! (list-ref (expval->vec vect) index1) (deref (list-ref (expval->vec vect) index2)))
                               (setref! (list-ref (expval->vec vect) index2) value))))
                         )
        ...
        (copy-vector-exp (exp1)
                         ...
                         (let ((vect (value-of exp1 env)))
                                 ...
                                 (let ((co (make-vector (length (expval->vec vect)) 1)))
                               
                                  

                                          (map update-help  co  (map deref (expval->vec vect)))   
                                    (vec-val co)))
        ...
        (vec-mult-exp (exp1 exp2)
                     ...
                     (let ((vect1 (expval->vec (value-of exp1 env)))
                       ...
                       (if (not (eq? (length vect1) (length vect2)))
                           (eopl:error "size did not match")
                      ...
                      (let ((vect11 (map deref vect1))
                         ...
                         (let ((co (make-vector (length  vect1) 1)))
                               
                                  

              (map update-help  co   (map num-val (map (lambda (x y) (* x y)) (map expval->num vect11) (map expval->num vect21))))
                            (vec-val co))))))
  ...
  (define (update-help index value)
